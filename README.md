...
### 1. Group Selector (`tools/group-selector/`)

Select all IFC objects that share a property value with a clicked object.

**Use Case:** Quickly select all walls of the same type, all doors with the same fire rating, all beams in a structural system, etc.

**Key Features:**
- Click any object in the 3D view
- Specify a property to match on (e.g., `ifcclass`, `Pset_WallCommon~FireRating`)
- All matching objects are selected and highlighted
- Shows match count

[📖 Group Selector README](tools/group-selector/README.md)

### 2. IFC Checker (`tools/ifc-checker/`)

Validate IFC model compliance against IDS (Information Delivery Specification) files.

**Use Case:** Enforce data quality standards — check that all walls have fire ratings, all doors have marks, structural elements have quantity parameters, etc.

**Key Features:**
- Load IDS specification files from URLs
- Run multi-specification checks against the model
- Color-code objects by compliance status (green = pass, red = fail)
- Interactive results — click failed objects to navigate to them
- Supports IDS 0.9.x and 1.0 formats
...
```
tools/
├── group-selector/       # Property-based object group selection
│   ├── src/
│   │   ├── index.ts
│   │   └── index.html
│   ├── package.json
│   ├── webpack.config.js
│   └── README.md
├── ifc-checker/          # IDS-based IFC model validation
│   ├── src/
│   │   ├── index.ts
│   │   ├── ids-parser.ts
│   │   ├── checker.ts
...
Each tool is independently buildable. To develop the group-selector widget:
...
The widget will be served on `http://localhost:3001` with hot reload.
...
3. **Create a widget instance** in StreamBIM:
   ...
   - Configure the widget settings (e.g., IDS file URL for ifc-checker)
...
2. Copy the structure from `tools/group-selector/` (or `tools/ifc-checker/`)
...
- **Issues & Feature Requests** — Open an issue on [GitHub](https://github.com/tikab-ab/0002-streambim-widget)
